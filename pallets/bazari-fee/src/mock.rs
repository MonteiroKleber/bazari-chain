use crate as pallet_bazari_fee;
...
parameter_types! {
	...
	pub const TreasuryAccountId: AccountId = 999;
	pub const MinOrderAmount: Balance = 100;
}

impl pallet_bazari_fee::Config for Test {
	type RuntimeEvent = RuntimeEvent;
	type Currency = Balances;
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Test> {
		balances: vec![
			...
			(account(4), 10_000),
			(account(5), 10_000),
			(account(999), 1), // Treasury starts at 1 (ExistentialDeposit)
		],
		..Default::default()
	}
